...
/*   Updated: 2025/10/15 18:25:21 by ruiferna         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "philo.h"

static void	ft_pick_forks(t_philo *philo, int *first, int *second)
{
	int	left;
	int	right;

	left = philo->id - 1;
	right = philo->id % philo->shared->num_philos;
	if (philo->id % 2 == 0)
	{
		*first = left;
		*second = right;
...
{
	ft_pick_forks(philo, first, second);
